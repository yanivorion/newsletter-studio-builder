...
export default function SocialLinksBlock({
  ...
  const activeLinks = Object.entries(links).filter(([, url]) => url && url !== '#' && url !== '');

  if (activeLinks.length === 0) {
    return (
      <div style={{ display: 'flex', justifyContent: alignMap[align], gap, padding: '8px 0' }}>
        {Object.keys(socialIcons).map((key) => (
          <span key={key} style={{ opacity: 0.4 }}>
            {socialIcons[key]?.(iconSize, iconColor)}
          </span>
        ))}
      </div>
    );
  ...
  return (
    <div style={{ display: 'flex', justifyContent: alignMap[align], gap, padding: '8px 0' }}>
      {activeLinks.map(([platform, url]) => (
        <a key={platform} href={url} style={{ display: 'inline-flex' }}>
          {socialIcons[platform]?.(iconSize, iconColor)}
        </a>
      ))}
    </div>
  );
